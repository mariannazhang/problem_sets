...
range(d$Game1Angry2, na.rm=TRUE)
...
filtered_d = filtered_d %>% 
  select(c("Subject", "Cond"), # Generally important columns for both hypotheses
         ...
         -contains("Intro"), -WhichGames, -GameComments, # except these
...
performance_diff_d = performance_hyp_long_d %>%
  ...
  select(-Score, -Measurement) %>% # now we remove columns we don't need (bonus: leave them in and see if you can understand what goes wrong!)
